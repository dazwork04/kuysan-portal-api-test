...
namespace SAPB1SLayerWebAPI.Controllers
{
    ...
    public class UserController : ControllerBase
    {
        ...
        [HttpPost("CreateUser/{userId}/{companyDB}")]
        public async Task<IActionResult> CreateUser(int userId, string companyDB, User user) => Ok(await userService.CreateUserAsync(userId, companyDB, user));

        [HttpPost("UpdateUser/{userId}/{companyDB}")]
        public async Task<IActionResult> UpdateUser(int userId, string companyDB, User user) => Ok(await userService.UpdateUserAsync(userId, companyDB, user));
    }
}
